...
cdef class Move:
    ...
    cdef public int from_square
    cdef public int to_square
    cdef public object promotion
    cdef public object drop
    ...
    def uci(self):
        ...
        if self.drop:
            ...
            # print("MOVE", self.from_square, self.to_square,
            #       SQUARE_NAMES[self.from_square] + SQUARE_NAMES[self.to_square])
...
cdef class BaseBoard:
    ...
    cdef public unsigned long long promoted
    cdef public unsigned long long occupied
    cdef public bint chess960
    cdef public object move_stack
    cdef public object stack
    cdef public object ep_square
    ...
    def _reset_board(self):
        ...
        # print("_reser_board", bbin(self.occupied))
    ...
    def _clear_board(self):
        ...
        self.occupied = BB_EMPTY
        #self.occupied = BB_RANK_1 | BB_RANK_2 | BB_RANK_7 | BB_RANK_8
        # print("_clear_board", bbin(self.occupied))
    ...
    def piece_at(self, int square):
        """Gets the :class:`piece <chess.Piece>` at the given square."""
        piece_type = self.piece_type_at(square)
        # print("piec_at square, piece", square, piece_type)
        if piece_type:
            mask = BB_SQUARES[square]
            color = bool(self.occupied_co[WHITE] & mask)
            # print("Piece", piece_type, color)
            return Piece(piece_type, color)

    def piece_type_at(self, int square):
        """Gets the piece type at the given square."""
        mask = BB_SQUARES[square]
        # print("Occupied", bbin(self.occupied), "id", id(self.occupied))
        # print("Pie mask", bbin(mask), bbin(self.occupied & mask))
    ...
    def _remove_piece_at(self, square):
        ...
        self.promoted &= ~mask
        # print("_remove_piece_at", bbin(self.occupied))
        return piece_type
    ...
    def _set_piece_at(self, square, piece_type, color, promoted=False):
        ...
        # print("_set_piece_at_1", square, piece_type, color, promoted)

        mask = BB_SQUARES[square]
        # print("_set_piece_at_2_mask", bbin(mask))

        if piece_type == PAWN:
            self.pawns |= mask
            # print("---> Pawns:", bbin(self.pawns))
        elif piece_type == KNIGHT:
            self.knights |= mask
            # print("---> Knigh:", bbin(self.knights))
        elif piece_type == BISHOP:
            self.bishops |= mask
            # print("---> Bisho:", bbin(self.bishops))
        elif piece_type == ROOK:
            self.rooks |= mask
            # print("---> Rooks:", bbin(self.rooks))
        elif piece_type == QUEEN:
            self.queens |= mask
            # print("---> Queen:", bbin(self.queens))
        elif piece_type == KING:
            self.kings |= mask
            # print("---> Kings:", bbin(self.kings))

        # print("Set Piece:", square, piece_type, color)
        # print("_set_piece_2_", bbin(self.occupied), bbin(mask))
        self.occupied ^= mask
        self.occupied_co[color] ^= mask

        # print("_set_piece_at", bbin(self.occupied))
    ...
    def _set_board_fen(self, fen):
        ...
        for c in fen:
            if c in ["1", "2", "3", "4", "5", "6", "7", "8"]:
                ...
                # print("Set Board Fen, Piece:", piece)
    ...
    def apply_transform(self, f):
        ...
        # print("apply_transform", bbin(self.occupied))
    ...
    def copy(self):
        ...
        # print("copy pawns:", bbin(board.pawns))
        # print("copy kings:", bbin(board.kings))
        # print("copy knigh:", bbin(board.knights))
        # print("copy bisho:", bbin(board.bishops))
        # print("copy rooks:", bbin(board.rooks))
        # print("copy queen:", bbin(board.queens))
        # print("copy occup:", bbin(board.occupied))
        # print("copy", bbin(self.occupied))
        return board
...
class _BoardState:
    ...
    def __init__(self, board):
        ...
        # print("_BoardState.__init__", bbin(self.occupied))
    ...
    def restore(self, board):
        ...
        board.occupied = self.occupied
        # print("_BoardSet", bbin(self.occupied))
        board.promoted = self.promoted
...
class Board(BaseBoard):
    ...
    def __init__(self, fen=STARTING_FEN, *, chess960=False):
        ...
        # self.occupied = BaseBoard.get_occupied()
        # print("Board.__init__", bbin(self.occupied), "id", id(self.occupied))
        # print(self.pawns)
        # print(self.knights)
        # print(self.bishops)
        # print(self.rooks)
    ...
    def push(self, Move move):
        ...
        # print("APPEND", self.occupied)
        # print("--->1", bbin(self.occupied))
        # print(self.occupied)
        # print(self.pawns)
        # print(self.kings)
        self.stack.append(_BoardState(self))
        # print("--->2", bbin(self.occupied))
    ...
    def _algebraic(self, Move move, long_san=False):
        if not move:
            # Null move.
            return "--"
    ...
    def is_irreversible(self, Move move):
        """
    ...
    def generate_legal_moves(self, from_mask=BB_ALL, to_mask=BB_ALL):
        if self.is_variant_end():
            return
        # print("generate_legal_move occupied", bbin(self.occupied))
        ...
        if king_mask:
            ...
            if checkers:
                ...
                for move in self.generate_pseudo_legal_moves(from_mask, to_mask):
                    if self._is_safe(king, blockers, move):
                        # print("generate:", str(move))
                        yield move
        else:
            # print("MOVE from", from_mask, to_mask)
    ...
    def _to_chess960(self, Move move):
